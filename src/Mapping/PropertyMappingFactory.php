...
namespace DevFarm\FilesUploaderBundle\Mapping;

use Doctrine\Common\Persistence\Proxy;
use Symfony\Component\DependencyInjection\ContainerInterface;
use DevFarm\FilesUploaderBundle\Exception\MappingNotFoundException;
use DevFarm\FilesUploaderBundle\Exception\NotUploadableException;
use DevFarm\FilesUploaderBundle\Metadata\MetadataReader;
use DevFarm\FilesUploaderBundle\Naming\ConfigurableInterface;
...
class PropertyMappingFactory
{
    ...
    protected function checkUploadable(string $class): void
    {
        if (!$this->metadata->isUploadable($class)) {
            throw new NotUploadableException(\sprintf('The class "%s" is not uploadable. If you use annotations to configure FilesUploaderBundle, you probably just forgot to add `@DevFarm\FilesUploaderBundle\Uploadable` on top of your entity. If you don\'t use annotations, check that the configuration files are in the right place. In both cases, clearing the cache can also solve the issue.', $class));
        }
    }
